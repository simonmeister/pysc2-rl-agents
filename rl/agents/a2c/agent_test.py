...
class A2CAgentTest(tf.test.TestCase):
  def test_compute_policy_log_probs(self):
    ...
    available_actions = A([[1, 0, 1], [1, 0, 0], [1, 1, 1]],
                          dtype=np.float32)
    fn_pi = A([[0.2, 0.0, 0.8], [1.0, 0.0, 0.0], [0.2, 0.7, 0.1]],
              dtype=np.float32)
    fn_ids = A([2, 0, 1], dtype=np.int32)
    arg_pi = {arg_type: A([[0.8, 0.2], [0.0, 1.0], [0.5, 0.5]],
                          dtype=np.float32)}
    ...
    expected_log_probs = np.log(
        [0.8, 1.0, 0.7]) + A([np.log(0.8), np.log(1.0), 0])
  ...
  def test_compute_policy_entropy(self):
    ...
    available_actions = A(
        ...
        dtype=np.float32)
    fn_pi = A(
        [[0.2, 0.0, 0.8], [1.0, 0.0, 0.0], [0.2, 0.7, 0.1]],
        dtype=np.float32)
    fn_ids = A([2, 0, 1], dtype=np.int32)
    arg_pi = {arg_type: A([[0.8, 0.2], [0.0, 1.0], [0.5, 0.5]],
                          dtype=np.float32)}
